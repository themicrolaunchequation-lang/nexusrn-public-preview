/* NexusRN v116 packaged-runtime-p2: nexus-preflight.pkg.js
   Generated from p1 packages in exact p1 load order. Originals retained. */

;/* ---- BEGIN pkg-00-manifest-guard.js ---- */
/* NexusRN v116 packaged runtime: Early manifest/runtime guard */
/* Generated non-destructively from original 63-script order. Originals retained. */


/* === BEGIN ORIGINAL SCRIPT: assets/js/nexusrn-v112-runtime-manifest-guard.js === */
(function(){
  'use strict';
  var CANONICAL_DB = 'data/questions-current.json';
  var DB_PATTERN = /(?:^|\/)(questions(?:-[^?#]+)?\.json)(?:[?#].*)?$/i;
  window.NEXUS_MANIFEST = window.NEXUS_MANIFEST || {version:'v112-bulk-authenticity-cleanup', canonicalDb: CANONICAL_DB};
  window.NEXUS_CURRENT_CANONICAL_DB = CANONICAL_DB;
  window.NEXUS_V111_CANONICAL_DB = CANONICAL_DB;
  ...
  if (typeof window.fetch === 'function') {
    ...
    function responseFromText(text){
      return new Response(text, { status: 200, headers: { 'Content-Type': 'application/json', 'X-NexusRN-DB': 'v112-bulk-authenticity-cleanup' } });
    }
      ...
      if (DB_PATTERN.test(clean) && !isCanonicalRequest) {
        ...
        if (!cachedDbTextPromise) {
          cachedDbTextPromise = nativeFetch(CANONICAL_DB, { cache: 'no-store' }).then(function(r){
            if (!r.ok) throw new Error('Canonical v108 DB failed: ' + r.status);
            return r.text();
          });
        }
  ...
  function runtimeAudit(){
    ...
    return {
      version: 'v112-bulk-authenticity-cleanup',
      ...
      note: 'v109 repairs duplicate-option P0 items and medication-safety wording; runtime manifest remains stable.'
    };
  }
...
try { window.NEXUS_V108_RUNTIME_AUDIT = window.NEXUS_V101_RUNTIME_AUDIT; } catch(e) {}

try { window.NEXUS_V109_RUNTIME_AUDIT = window.NEXUS_V101_RUNTIME_AUDIT; } catch(e) {}

try { window.NEXUS_V111_RUNTIME_AUDIT = window.NEXUS_V101_RUNTIME_AUDIT; window.NEXUS_V110_RUNTIME_AUDIT = window.NEXUS_V101_RUNTIME_AUDIT; } catch(e) {}

try { window.NEXUS_V111_RUNTIME_AUDIT = window.NEXUS_V101_RUNTIME_AUDIT; } catch(e) {}

try { window.NEXUS_V112_RUNTIME_AUDIT = window.NEXUS_V101_RUNTIME_AUDIT; } catch(e) {}
/* === END ORIGINAL SCRIPT: assets/js/nexusrn-v112-runtime-manifest-guard.js === */

;/* ---- END pkg-00-manifest-guard.js ---- */
